      ...
      <div className="flex-1 flex flex-col max-w-3xl w-full mx-auto">
        {!sessionStarted ? (
          <div className="flex-1 flex flex-col items-center justify-center px-6 py-16">
            ...
            {sttAvailable && (
              <button
                onClick={micPermission === "denied" ? undefined : handleMicClick}
                disabled={micPermission === "denied"}
                title={micPermission === "denied" ? "Microphone access blocked — enable it in your browser settings" : isListening ? "Stop listening" : "Tap to speak"}
                className={`w-16 h-16 rounded-full flex items-center justify-center mb-3 transition-all ${
                  micPermission === "denied"
                    ? "bg-white/[0.05] text-white/20 border border-white/10 cursor-not-allowed"
                    : isListening
            ...
            {micPermission === "denied" && (
              <div className="flex items-center gap-2 bg-red-soft/10 border border-red-soft/30 rounded-xl px-4 py-2.5 mb-5 max-w-sm text-center">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-red-soft shrink-0">
                  <circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" />
                </svg>
                <span className="text-[12px] text-white/50">
                  Microphone blocked. Enable it in your browser{" "}
                  <span className="text-white/70">Settings → Site permissions → Microphone</span>.
                </span>
              </div>
            )}
          ...
          <div className="flex-1 overflow-y-auto px-4 lg:px-0 py-6">
            {resuming && (
              <div className="bg-saffron/10 border border-saffron/20 rounded-xl px-4 py-3 text-center mb-6">
